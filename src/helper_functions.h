...
inline double multivariate_norm(double obs_x, double landmark_x, double obs_y, double landmark_y, double std_landmark_x, double std_landmark_y) {
	//std::cout << "HELLO OH MY GOD" << std::endl;
	double exp1 = pow((obs_x - landmark_x),2)/(2*pow(std_landmark_x,2));
	double exp2 = pow((obs_y - landmark_y),2)/(2*pow(std_landmark_y,2));
	double norm = 1/(2*M_PI*std_landmark_x*std_landmark_y);
